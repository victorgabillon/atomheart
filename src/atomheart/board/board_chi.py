    ...
    def copy(
        ...
        Args:
            copied_chess_board (chess.Board | None, optional): The chess board to use for the copy.
                If None, the current chess board is used. Defaults to None.

        Returns:
            LegalMoveKeyGenerator: A copy of the LegalMoveKeyGenerator.
        ...
        if self.all_generated_keys_ is not None:
            # works for any Sequence (list/tuple/etc)
            legal_move_copy.all_generated_keys_ = self.all_generated_keys_.copy()
        else:
            legal_move_copy.all_generated_keys_ = None
    ...
    def get_all(self) -> list[MoveKey]:
        ...
        if self.all_generated_keys is None:
            if self.sort_legal_moves:
                ...
                list_keys = list(self)
            self.all_generated_keys_ = list_keys
            return list_keys
...
class BoardChi(IBoard):
    ...
    def play_move(
        ...
        """
        # TODO(victor): illegal moves seem accepted, do we care? if we dont write it in the doc. See issue #24 for more details. See issue #24 for more details.
    ...
    def push_and_return_modification(
        ...
        if piece_type == chess.KING and not promoted:
            ...
            if self.turn == Color.WHITE and chess.square_rank(move.to_square) == 7:
                self.chess_board.castling_rights &= ~chess.BB_RANK_8
            elif self.turn == Color.BLACK and chess.square_rank(move.to_square) == 0:
                self.chess_board.castling_rights &= ~chess.BB_RANK_1
        ...
        if piece_type == chess.PAWN:
            ...
            if diff == 16 and chess.square_rank(move.from_square) == 1:
                ...
                down = -8 if self.turn == Color.WHITE else 8
        ...
        if castling:
            ...
            if a_side:
                ...
                rook_square = (
                    chess.D1 if self.chess_board.turn == Color.WHITE else chess.D8
                )
                self.chess_board._set_piece_at(
                    chess.C1 if self.turn == Color.WHITE else chess.C8,
                    chess.KING,
                    self.turn,
                )
                self.chess_board._set_piece_at(
                    chess.D1 if self.turn == Color.WHITE else chess.D8,
                    chess.ROOK,
                    self.turn,
                )
            else:
                ...
                rook_square = (
                    chess.F1 if self.chess_board.turn == chess.WHITE else chess.F8
                )
                self.chess_board._set_piece_at(
                    chess.G1 if self.turn == Color.WHITE else chess.G8,
                    chess.KING,
                    self.turn,
                )
                self.chess_board._set_piece_at(
                    chess.F1 if self.turn == Color.WHITE else chess.F8,
                    chess.ROOK,
                    self.turn,
                )
    ...
    def compute_key_old(self) -> str:
        ...
        return (
            ...
            + str(self.chess_board.occupied_co[bool(Color.WHITE)])
            + str(self.chess_board.occupied_co[bool(Color.BLACK)])
    ...
    def turn(self) -> Color:
        ...
        """
        return Color(self.chess_board.turn)
    ...
    def piece_map(
        ...
        for square in chess.scan_reversed(self.chess_board.occupied & mask):
            ...
            color = bool(self.chess_board.occupied_co[bool(Color.WHITE)] & mask)
